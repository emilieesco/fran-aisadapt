...
-   **Outils d'accessibilité avancés :** Panneau flottant avec 5 outils : (1) Mode dyslexie — police Atkinson Hyperlegible + espacement des lettres/mots, persisté en localStorage + data-dyslexia sur `<html>`; (2) Taille du texte (Normal/Grand/Très grand via data-font-size); (3) Interlignage ajustable (Normal/Grand/Très grand via data-line-spacing CSS); (4) Vitesse de lecture TTS (Lente 0.68 / Normale 0.88 / Rapide 1.2) — respectée par tous les boutons d'écoute et les dictées; (5) Prédicteur de mots — suggestions de complétion pour les exercices à trous (fill_blank) affichées dès 2 lettres tapées; source combinée = toutes les réponses correctes de l'exercice (toutes ses questions fill_blank) + vocabulaire global français (~2 600+ entrées uniques : déterminants, pronoms, prépositions, conjonctions, connecteurs, adverbes, adjectifs, infinitifs, formes conjuguées présent/passé/futur/conditionnel, participes, grammaire, orthographe/homophones, narratif, descriptif, explicatif, argumentatif, écriture, vocabulaire scolaire, quotidien, FPT québécois, lecture, marqueurs de relation, expressions québécoises) défini dans `client/src/lib/french-vocabulary.ts`.
